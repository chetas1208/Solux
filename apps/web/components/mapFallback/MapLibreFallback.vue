...
<template>
  <div class="relative w-full h-full min-h-[320px] bg-zinc-900 overflow-hidden">
    <EarthGlobeUnavailableState
      v-if="!hasMapProvider"
      title="2D fallback unavailable"
      message="Set NUXT_PUBLIC_MAPTILER_KEY for MapLibre basemaps."
    />
    <EarthGlobeUnavailableState v-else-if="mapError" :message="mapError" />
    <div v-else ref="container" class="absolute inset-0" />
    <EarthGlobeLegend class="absolute bottom-3 left-3" />
  </div>
</template>
